...
const int MAX_HEIGHT = MAX_ITEM_NUM;
    ...
    bool hasOverlap(int x, int y, Mino m){
        for(int i = 0; i < m.getHeight(); i++){
            for(int j = 0; j < m.getWidth(); j++){
                //if(m.get(i, j) == '#' && board[x+i][y+j] == '#') return true;
                if(m.get(i, j) == '#' && board.test((x * 5 + y) + j + 5 * i)) return true;
            }
        }
        return false;
    ...
    bool addMino(int x, int y, Mino m){
        ...
        for(int i = 0; i < m.getHeight(); i++){
            for(int j = 0; j < m.getWidth(); j++){
                if(m.get(i, j) == '#'){
                    //board[x + i][y + j] = '#';
                    //board.set(j + MAX_WIDTH * (i - 1), 1);
                    board.set((x * 5 + y) + j + 5 * i, 1);
                }
            }
        }
        count++;
...
public:
    KataminoZDD(vector<Mino> minos): minos(minos){
    }
